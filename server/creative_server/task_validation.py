...
MEDIA_INPUT_OPERATIONS = {"image_edit", "image_to_video", "continue_video", "extract_video_frames", "video_breakdown", "video_edit"}
...
def _safe_external_video_references(inputs: dict) -> list[dict]:
    values = inputs.get("reference_assets", []) if isinstance(inputs, dict) else []
    result = []
    for value in values if isinstance(values, list) else []:
        if not isinstance(value, dict) or str(value.get("role") or "") != "reference_video":
            continue
        parsed = urlparse(str(value.get("path") or ""))
        if parsed.scheme == "https" and parsed.netloc and not parsed.username and not parsed.password:
            result.append(value)
    return result


def validate_task_request(db: Session, project_id: str, kind: str, provider: str,
                          task_input: dict, model: str = "") -> None:
    ...
    external_video_references = _safe_external_video_references(inputs)
    ...
    if provider == "seedance" and model:
        try:
            from ai.providers.video.seedance_models import seedance_model_profile
            reference_limit = int(seedance_model_profile(model).get("reference_assets") or reference_limit)
        except Exception:
            pass
    ...
    if (kind in MEDIA_INPUT_OPERATIONS and
            (not isinstance(references, list) or not references) and
            not (kind == "video_edit" and external_video_references)):
