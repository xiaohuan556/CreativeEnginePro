...
export const NODE_SPECS = [
  spec({ key: "storyboard", desktopType: "storyboard_node", title: "AI 故事板", description: "一句故事自动拆镜并完成制片", kind: "storyboard", group: "primary", accent: "#89b8ff", creatable: true, input: ["text"], output: ["shot", "image", "video", "audio"], actions: [], defaults: { style: "电影写实", shot_count: 0, automation_mode: "checkpoints", production_scope: "all", production_ratio: "16:9", pipeline_stage: 0 } }),
  ...
  spec({ key: "multi_image", desktopType: "image_node", variant: "multi_image_composer", title: "多图生成图片", description: "为每张参考图指定主体、场景、构图、元素或风格用途", kind: "image", group: "primary", accent: "#50b9dd", creatable: true, input: ["image", "asset"], output: ["image"], actions: ["设置每张图片的用途", "AI 编辑", "保存到资产库"], defaults: { multi_image_composer: true, references: [], reference_assets: [], reference_settings: [], editor_action: "AI 编辑", ratio: "16:9", candidate_count: 1, style_preset: "", style_recipe: null, style_custom: "", style_scope: "whole", style_strength: 70, style_preserve: ["identity", "pose", "composition", "background", "lighting"] } }),
  ...
  spec({ key: "multi_director", desktopType: "video_node", variant: "multi_image_director", title: "多图导演视频", description: "按时间段、动作、运镜和用途一次生成完整视频", kind: "director", group: "primary", accent: "#6f8cff", creatable: true, input: ["image", "asset"], output: ["video"], actions: ["图生视频", "基于尾帧续拍", "提取首中尾帧"], defaults: { multi_image_director: true, timeline_images: [], references: [], reference_assets: [], duration: 10, ratio: "16:9", resolution: "720p", generate_audio: true, audio_prompt: "对白、环境声和动作声与画面同步，不使用背景音乐掩盖对白", generator_kind: "video", editor_action: "图生视频" } }),
  ...
  spec({ key: "shot", desktopType: "shot", title: "镜头节点", description: "镜头提示词、参考、关键帧、视频与对白", kind: "shot", group: "basic", accent: "#6f8cff", creatable: true, input: ["image", "asset", "shot"], output: ["shot", "image", "video", "audio"], actions: ["生成关键帧", "参考图再生成", "生成视频", "生成对白", "保存镜头修改"], defaults: { editor_action: "生成关键帧", shot: {}, assets: [] } }),
  ...
  spec({ key: "scene_reference", desktopType: "image_node", variant: "scene", title: "场景参考", description: "描述场景或上传参考图", kind: "reference", group: "reference", accent: "#48d597", creatable: true, input: ["image"], output: ["asset", "image"], actions: ["文生图", "图生图", "保存到资产库"], defaults: { asset_kind: "scene", reference_role: "scene", ratio: "16:9", editor_action: "文生图" } }),
  spec({ key: "character_reference", desktopType: "image_node", variant: "character", title: "主体参考", description: "描述角色主体或上传参考图", kind: "reference", group: "reference", accent: "#b98cff", creatable: true, input: ["image"], output: ["asset", "image"], actions: ["文生图", "图生图", "保存到资产库"], defaults: { asset_kind: "character", reference_role: "character", ratio: "2:3", editor_action: "文生图" } }),
  spec({ key: "element_reference", desktopType: "image_node", variant: "element", title: "元素参考", description: "描述道具元素或上传参考图", kind: "reference", group: "reference", accent: "#4fc4e8", creatable: true, input: ["image"], output: ["asset", "image"], actions: ["文生图", "图生图", "保存到资产库"], defaults: { asset_kind: "element", reference_role: "element", ratio: "1:1", editor_action: "文生图" } }),
...
/** Acceptance matrix shared by UI audits and contract tests. */
export const NODE_EXECUTION_CONTRACTS = {
  storyboard: { mode: "production", operations: ["production_run"] },
  script: { mode: "task", operations: ["chat"] },
  copywriting: { mode: "task", operations: ["chat"] },
  multi_image: { mode: "task", operations: ["text_to_image", "image_edit"] },
  image_asset: { mode: "direct", operations: ["derive_image", "derive_video", "save_to_library"] },
  multi_director: { mode: "task", operations: ["text_to_video", "image_to_video", "continue_video", "extract_video_frames"] },
  video: { mode: "task", operations: ["text_to_video", "image_to_video", "continue_video", "extract_video_frames"] },
  audio: { mode: "task", operations: ["text_to_speech"] },
  shot: { mode: "task", operations: ["text_to_image", "image_edit", "text_to_video", "image_to_video", "text_to_speech"] },
  analysis: { mode: "task", operations: ["video_breakdown", "export_report"] },
  skill: { mode: "expand", operations: ["storyboard", "motion_grid", "camera_grid", "continuity_grid", "character_grid", "lighting_grid", "emotion_grid"] },
  scene_reference: { mode: "task", operations: ["text_to_image", "image_edit", "save_to_library"] },
  character_reference: { mode: "task", operations: ["text_to_image", "image_edit", "save_to_library"] },
  element_reference: { mode: "task", operations: ["text_to_image", "image_edit", "save_to_library"] },
  project: { mode: "root", operations: [] },
  legacy_scene: { mode: "direct", operations: ["save_to_library"] },
  legacy_character: { mode: "direct", operations: ["save_to_library"] },
  legacy_element: { mode: "direct", operations: ["save_to_library"] },
  asset_view: { mode: "review", operations: ["adopt", "reject"] },
  asset_take: { mode: "review", operations: ["adopt", "reject", "accept_risk"] },
  workflow: { mode: "workflow", operations: ["run", "pause", "resume", "cancel", "save_template"] },
  task: { mode: "queue", operations: ["pause", "resume", "cancel", "retry"] },
  result: { mode: "review", operations: ["adopt", "reject", "accept_risk", "save_to_library"] },
} as const satisfies Record<(typeof NODE_SPECS)[number]["key"], { mode: string; operations: readonly string[] }>;
...
export const CREATION_GROUPS: Array<{ key: NodeGroup; label: string }> = [
  { key: "primary", label: "从想法开始" },
  { key: "basic", label: "生成图片、视频和声音" },
  { key: "analysis", label: "视频分析与高级工具" },
];

export function buildSkillPrompts(action: string, instruction: string) {
  if (action === "调度与运动分镜") {
    return [
      "建立空间与人物初始位置", "人物开始第一个明确动作", "动作推进并建立视线方向",
      "人物与关键道具发生交互", "镜头跟随主体完成空间移动", "动作转折与重心变化",
      "人物抵达目标位置", "关键情绪反应", "动作结束并形成可续接姿态",
    ].map((beat, index) => `${instruction}。调度分镜第 ${index + 1}/9 格：${beat}。只推进一个动作节拍；标明人物位置、朝向、运动轨迹和主运镜；严格保持身份、服装、场景轴线与光线连续。`);
  }
  if (action === "多机位九宫格") {
